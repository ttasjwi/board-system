...
class RefreshTokenHandler(
    ...
    private val userRefreshTokenIdListPersistencePort: UserRefreshTokenIdListPersistencePort,
    ...
    companion object {
        ...
        internal const val MAX_USER_REFRESH_TOKEN_COUNT = 5
    }

    /**
     * 리프레시토큰을 생성하고, 저장소에 보관합니다.
     */
    fun createAndPersist(userId: Long, issuedAt: AppDateTime): RefreshToken {
        val refreshToken = generate(userId, issuedAt)
        save(refreshToken)
        return refreshToken
    }
    ...
    fun throwIfExpired(refreshToken: RefreshToken, currentTime: AppDateTime) {
        if (refreshToken.isExpired(currentTime)) {
            userRefreshTokenIdListPersistencePort.remove(refreshToken.userId, refreshToken.refreshTokenId)
            throw RefreshTokenExpiredException("리프레시 토큰 만료시간이 경과되어 만료됨")
        }

        // 회원 리프레시토큰 Id 목록에 있는 지 확인(로그아웃, 동시 유지 가능 갯수 제한 등으로 인해 사라졌을 수 있음)
        // 없다면, 예외 발생
        if (!userRefreshTokenIdListPersistencePort.exists(refreshToken.userId, refreshToken.refreshTokenId)) {
    ...
    fun refreshIfRequired(refreshToken: RefreshToken, currentTime: AppDateTime): RefreshToken {
        if (currentTime >= refreshToken.expiresAt.minusHours(REFRESH_REQUIRE_THRESHOLD_HOURS)) {
            remove(refreshToken.userId, refreshToken.refreshTokenId)
            return createAndPersist(refreshToken.userId, currentTime)
        }
        return refreshToken
    }

    private fun generate(userId: Long, issuedAt: AppDateTime): RefreshToken {
    ...
    private fun save(refreshToken: RefreshToken) {
        ...
        // 회원의 기존 토큰 ID 목록 조회
        val refreshTokenIds = userRefreshTokenIdListPersistencePort.findAll(userId)
        ...
        for (tokenId in refreshTokenIds) {
            if (!refreshTokenIdPersistencePort.exists(userId, tokenId)) {
                userRefreshTokenIdListPersistencePort.remove(userId, tokenId)
                deleteCount++
            }
        }
        ...
        // 가장 오래된 토큰을 제거
        if (idCount == MAX_USER_REFRESH_TOKEN_COUNT && deleteCount == 0) {
            remove(userId, refreshTokenIds[0])
        }

        // 새로운 토큰 저장
        userRefreshTokenIdListPersistencePort.add(
            userId, refreshToken.refreshTokenId, MAX_USER_REFRESH_TOKEN_COUNT.toLong()
        )
        refreshTokenIdPersistencePort.save(
            userId, refreshToken.refreshTokenId, refreshToken.expiresAt
        )
    }

    private fun remove(userId: Long, refreshTokenId: Long) {
        userRefreshTokenIdListPersistencePort.remove(userId, refreshTokenId)
        refreshTokenIdPersistencePort.remove(userId, refreshTokenId)
    }
}
