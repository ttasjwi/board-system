...
import com.ttasjwi.board.system.global.springsecurity.token.X509CertificateThumbprintValidator
...
class NimbusJwtConfig(
    @Value("\${rsa-keypair.public-key-path}")
    private val rsaPublicKeyPath: String,
    @Value("\${rsa-keypair.private-key-path}")
    private val rsaPrivateKeyPath: String,
    ...
    private fun loadRSAPublicKey(): RSAPublicKey {
        val resource = resourceLoader.getResource(rsaPublicKeyPath)
    ...
    private fun loadRSAPrivateKey(): RSAPrivateKey {
        val resource = resourceLoader.getResource(rsaPrivateKeyPath)
