...
class AccessTokenAuthenticationFilterTest {
    ...
    fun testNullTokenValue() {
        ...
        verify(exactly = 1) { filterChain.doFilter(request, response) }
        verify(exactly = 0) { accessTokenGenerator.parse(anyNullable()) }
    }
    ...
    fun testWithValidAccessToken() {
        ...
        every { accessTokenGenerator.parse(tokenValue) } returns accessToken
        every { timeManager.now() } returns currentTime
        every { accessTokenGenerator.checkCurrentlyValid(accessToken, currentTime) } just Runs
        every { filterChain.doFilter(request, response) } just Runs
        ...
        // then
        verify(exactly = 1) { accessTokenGenerator.parse(tokenValue) }
        verify(exactly = 1) { bearerTokenResolver.resolve(request) }
        verify(exactly = 1) { timeManager.now() }
        verify(exactly = 1) { accessTokenGenerator.checkCurrentlyValid(accessToken, currentTime) }
        verify(exactly = 1) { filterChain.doFilter(request, response) }
    }
