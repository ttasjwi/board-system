...
import com.ttasjwi.board.system.auth.domain.external.fixture.AccessTokenGeneratorFixture
...
class JwtAccessTokenManagerTest {
    ...
    fun setup() {
        accessTokenGenerator = AccessTokenGeneratorImpl(
            AccessTokenGeneratorFixture()
        )
    }

    companion object {
        private val log = getLogger(JwtAccessTokenManagerTest::class.java)
    }
    ...
    inner class Generate {
        ...
        fun test() {
            ...
            // when
            val accessToken = accessTokenGenerator.generate(authMember, issuedAt)
    ...
    inner class Parse {
        ...
        fun testSuccess() {
            ...
            val authMember = authMemberFixture()
            val issuedAt = appDateTimeFixture(minute = 5)
            val tokenValue = accessTokenGenerator.generate(authMember, issuedAt).tokenValue

            // when
            val accessToken = accessTokenGenerator.parse(tokenValue)
    ...
    inner class CheckCurrentlyValid {
        ...
        fun testValid() {
            ...
            // when
            // then
            accessTokenGenerator.checkCurrentlyValid(accessToken, currentTime)
        }
        ...
        fun testExpired2() {
            ...
            // then
            assertThrows<AccessTokenExpiredException> {
                accessTokenGenerator.checkCurrentlyValid(
                    accessToken,
                    currentTime
                )
            }
